...
**Versão:** 1.0.1 | **Data:** 2026-08-18 | **Domínio:** architecture | **Autor:** Pedro O. Silva
...
### 5.1 Idempotência (Propriedade Arquitetural, NÃO Guardrail)

**IMPORTANTE**: Idempotência não é um guardrail — é uma **propriedade arquitetural**
que decorre da escolha da estratégia de gravação. Guardrails *validam* dados;
idempotência *define como o pipeline se comporta* ao rodar múltiplas vezes.

Toda escrita a jusante deve ser idempotente: rodar duas vezes o mesmo período não
pode duplicar nem divergir.

**Como implementar idempotência por camada:**

| Camada | Estratégia | Idempotente? | Como |
|--------|-----------|--------------|------|
| **Bronze** | APPEND puro + versionamento | ❌ NÃO (por design) | Preserva histórico completo; filtro em Silver |
| **Bronze** | APPEND + verificação prévia | ✅ SIM | Checar se já processou antes de rodar |
| **Silver/Gold** | DELETE WHERE + APPEND | ✅ SIM | Remove período, depois insere |
| **Silver/Gold** | MERGE (upsert) | ✅ SIM | Atualiza existente, insere novo |
| **Silver/Gold** | replaceWhere | ✅ SIM | Substitui partição atomicamente |
| **Silver/Gold** | APPEND puro | ❌ NÃO | Duplica em reprocessamento (ANTIPADRÃO) |

**Exemplo - Tornar Bronze idempotente via verificação prévia:**
...
**Trade-off Bronze**: APPEND puro preserva todo histórico mas não é idempotente.
Útil quando você quer rastrear *quando* cada versão chegou. Para idempotência,
adicione verificação de estado ou use MERGE.

Padrões: `MERGE` por chave+competência, ou overwrite particionado (`replaceWhere`).
Append cego em camadas refinadas é antipadrão.
