...
Este documento COMPLEMENTA `arquitetura-medalhao/SKILL.md` (seções 5.1 Idempotência, 5.3 Qualidade, 5.4 Observabilidade). Enquanto o medalhão define **o que cada camada faz**, este documento define **como garantir que falhas não destruam a execução**.
...
- [ ] **Idempotência**: Rodar N vezes o mesmo período produz o mesmo resultado? (ver `arquitetura-medalhao/SKILL.md` seção 5.1)
...
## 8. Aplicação em Notebooks Databricks (Resiliência sem Quebrar Estrutura de Células)

**Ver também**: `estrutura-notebooks/SKILL.md` — separação de responsabilidades por célula.

### 8.1 Por que "try numa célula, except em outra" não funciona

Cada célula de um notebook Databricks é compilada e executada **isoladamente** (mesmo mecanismo do Jupyter). Um bloco `try:` aberto em uma célula sem o `except`/`finally` correspondente na MESMA célula é `SyntaxError`/`IndentationError` no momento em que essa célula roda sozinha — não existe "bloco try que atravessa células".

**Antipadrão (não executa)**:
```python
...
Mantém a separação de responsabilidades (cada etapa é uma função definida na sua própria célula, com título em MAIÚSCULO) **e** a resiliência (o loop com try/except vive inteiro em uma única célula, que apenas chama as funções):
...
* **Idempotência**: Ver `arquitetura-medalhao/SKILL.md` seção 5.1
* **Guardrails por camada**: Ver `arquitetura-medalhao/SKILL.md` seção 5.3
* **Observabilidade**: Ver `arquitetura-medalhao/SKILL.md` seção 5.4
* **Estratégias de gravação**: Ver `arquitetura-medalhao/SKILL.md` seção 5.5
